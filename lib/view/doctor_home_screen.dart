...
import 'package:hospital_app/view/doctor_profile_screen.dart';
...
class _DoctorHomeScreenState extends State<DoctorHomeScreen> {
  ...
  Future<void> getData()async{
    ...
    await provider.getAppointments(auth.userId!,'doctor', auth.token!);
    setState(() {
      loading = false;
    });
  }
  ...
  Widget build(BuildContext context) {
   var appointments =  Provider.of<AppointmentViewModel>(context,listen: false).list;
    return Scaffold(
      appBar: AppBar(
        actions: [
          IconButton(onPressed: (){Navigator.push(context, MaterialPageRoute(builder: (context) => DoctorProfileScreen(),));}, icon: const Icon(Iconsax.profile_circle)),
        ],
        title: const Text("Today's Appointments",style: TextStyle(fontWeight: FontWeight.bold),),
      ),
...
Widget appointmentItem(PrescriptionParams params,BuildContext context){
  ...
  return GestureDetector(
    onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => AppointmentDetailsScreen(params: params,done: done),)),
    child: Container(
      ...
      child: Center(
        child: ListTile(
          ...
          subtitle: Text(done? 'Appointment Finished' : 'You have a new Appointment',style: const TextStyle(fontSize: 14,fontWeight: FontWeight.bold),),
